---
title: "LPL lab mussel salinity trials 2023"
output: 
  html_document:
    keep_md: yes
editor_options: 
...
localpath = '../../data/Lab_mussel_salinity_trials/LPL_Mytilus_lab_salinity_trials_Summer2023/'  # Luke's path from the R code dir to the data dir
...
Trial 1 consisted of these mussels: 
Low salinity (3PSU): SN189 SN218 SN179 (SN103 didn't record data during trial)
Control salinity: SN224 SN196 SN173 (missing a 4th mussel)

Trial 1 started the salinity decline 2023-08-23 09:00 PDT
Trial 1 finished the salinity rise 2023-08-29 15:20 PDT


Trial 2 had these mussels:
Low salinity (3PSU): SN245 SN226 SN243 SN198
Control salinity: SN217 SN214 SN242  SN151

Trial 2 started the salinity decline 2023-09-19 10:15PDT
Trial 2 finished the salinity rise 2023-09-20 19:10PDT
...
trialMetaData$AcclimationStartTimePDT = 
		lubridate::force_tz(trialMetaData$AcclimationStartTimePDT, tzone = 'PST8PDT')
trialMetaData$TrialStartTimePDT = 
		lubridate::force_tz(trialMetaData$TrialStartTimePDT, tzone = 'PST8PDT')
trialMetaData$TrialStopTimePDT = 
		lubridate::force_tz(trialMetaData$TrialStopTimePDT, tzone = 'PST8PDT')
...
for (i in 1:length(ids)){
	...
	# Grab the baseline Hall effect value when the shell is fully closed (0mm)
	tempBaselineHall = mean(temp$HallReading[which(temp$GaugeDiameter.mm == 0)])
...
#' a column 'Intercept' and column 'Slope' containing coefficients from a
#' linear regression of Hall readings vs. known gape distance, and a column 
#' 'BaselineHall' that contains the Hall effect value when the shell was fully 
...
HalltoMM = function(GapeData, CalibFits) {
	...
	# in CalibFits.
	tempSlope = CalibFits$Slope[which(CalibFits$SerialNumber == GapeData$SN[1])]
	tempIntercept = CalibFits$Intercept[which(CalibFits$SerialNumber == GapeData$SN[1])]
	tempBaselineHall = CalibFits$BaselineHall[which(CalibFits$SerialNumber == GapeData$SN[1])]
	
	# First deal with any potential offset in the baseline Hall reading. If 
	# the minimum Hall value in the dataset is different than the minimum value
	# from the calibrations, adjust the trial data up or down by that difference
	# so that they will start at 0mm. 
	
	trialMinimumHall = min(GapeData$Hall, na.rm=TRUE)
	halldiff = tempBaselineHall - trialMinimumHall
	# If the tempBaselineHall is larger than trialMinimumHall, adjust all 
	# trial values upwards by that amount, then apply the calibration regression
	
	GapeData$Halladj = GapeData$Hall + halldiff
	
	GapeData$gape.mm = (GapeData$Halladj * tempSlope) + tempIntercept
...
```{r ConcatGapeFilesFunction,echo=showcode}
################################################################
# ConcatGapeFiles
...
# Open the raw data files and concatenate them.
	for (f in 1:length(filenames)){
		...
		if (length(line2) > 0){
		  ...
		  # Use lubridate package to parse the DateTime values. This should 
		  # handle both yyyy-mm-dd format (default from the datalogger) and Excel's mm/dd/yy format
			dattemp$DateTime = lubridate::parse_date_time(dattemp$DateTime, 
			                                              orders = c('%Y-%m-%d %H:%M:%S', '%m/%d/%Y %H:%M:%S'), 
			                           tz = myTimeZone)
...
```{r ConcatHeartFilesFunction,echo=showcode}
################################################################
...
```{r importSalinityData,echo=showcode, cache=TRUE}
#############################################################
# Import the salinity/temp/DO data file

waterdata = readxl::read_excel(paste0(localpath,trialfilename),
		sheet = 'SalinityTempData')
# DateTime is imported as UTC, but was recorded in the original file
# as PDT, so change the associated time zone
waterdata$DateTime = lubridate::force_tz(waterdata$DateTime, 
		tzone = 'PST8PDT')

# Drop the Notes column
waterdata = waterdata[,-which(colnames(waterdata)=='Notes')]

# It will probably be easiest to refer to these by their data logger
# serial numbers, so merge the SNs into this data frame
waterdata = merge(waterdata, trialMetaData[,c('MusselID','SerialNumber')], by.x = 'MusselID', by.y = 'MusselID')
# Reorder by time of observation
waterdata = waterdata[order(waterdata$DateTime),]

waterdata$SalinityTreatment = factor(waterdata$SalinityTreatment)
waterdata$TrialNumber = factor(waterdata$TrialNumber)

```

```{r plotSalinitydataTrial1,echo=showcode}

# Trial 1

tr1 = waterdata[which(waterdata$TrialNumber == '1'),]
tr1 = droplevels(tr1)

plot(tr1$DateTime, y = tr1$Salinity_psu, type='n',
		ylab = 'Salinity, psu', las = 1, 
		xlab = 'Time')
for (i in 1:length(levels(tr1$SerialNumber))){
	thisSN = levels(tr1$SerialNumber)[i]
	dat = tr1[which(tr1$SerialNumber == thisSN),]
	
	if(dat$SalinityTreatment[1] == '3psu') {
		mycol = 3
	} else if (dat$SalinityTreatment[1] == 'control') {
		mycol = 4
	}
	
	lines(dat$DateTime, dat$Salinity_psu, col = mycol)
}
legend('bottomleft', legend = c('Control','3 psu'), col = c(4,3),
		lty = c(1,1))
mtext(side = 3, text = 'Trial 1')

declineStart = as.POSIXct('2023-08-28 14:20',tz='PST8PDT')
declineFinish = as.POSIXct('2023-08-28 22:30',tz='PST8PDT')
riseStart = as.POSIXct('2023-08-29 11:40',tz='PST8PDT')
riseFinish= as.POSIXct('2023-08-29 15:10',tz='PST8PDT')

#abline(v = c(declineStart, declineFinish,riseStart,riseFinish))

```


```{r plotSalinitydataTrial2,echo=showcode}

# Trial 2

tr2 = waterdata[which(waterdata$TrialNumber == '2'),]
tr2 = droplevels(tr2)

plot(tr2$DateTime, y = tr2$Salinity_psu, type='n',
		ylab = 'Salinity, psu', las = 1, 
		xlab = 'Time')
for (i in 1:length(levels(tr2$SerialNumber))){
	thisSN = levels(tr2$SerialNumber)[i]
	dat = tr2[which(tr2$SerialNumber == thisSN),]
...
legend('bottomleft', legend = c('Control','3 psu'), col = c(4,3),
		lty = c(1,1))
mtext(side = 3, text = 'Trial 2')

declineStart2 = as.POSIXct('2023-09-19 10:00',tz='PST8PDT')
declineFinish2 = as.POSIXct('2023-09-19 15:20',tz='PST8PDT')
riseStart2 = as.POSIXct('2023-09-20 10:00',tz='PST8PDT')
riseFinish2 = as.POSIXct('2023-09-20 19:10',tz='PST8PDT')

abline(v = c(declineStart2, declineFinish2,riseStart2,riseFinish2))

declineFinish2 - declineStart2
riseFinish2 - riseStart2
...
```{r loadConcatGapeDataTrial1, echo=showcode, cache=TRUE}
...
SN173gape = HalltoMM(SN173gape,CalibFits)  # control
SN196gape = HalltoMM(SN196gape, CalibFits) # control
SN224gape = HalltoMM(SN224gape, CalibFits) # control
SN189gape = HalltoMM(SN189gape, CalibFits) # 3psu
SN218gape = HalltoMM(SN218gape, CalibFits) # 3psu
SN179gape = HalltoMM(SN179gape, CalibFits) # 3psu
...
quickplot2 = function(GapeData){
	plot(GapeData$DateTime, y = GapeData$gape.mm, type = 'l')
	abline(h = 0, lty = 2)
}

# quickplot2(SN173gape)
# quickplot2(SN196gape)
# quickplot2(SN189gape)
# quickplot2(SN218gape)
...
```{r loadConcatGapeDataTrial2, echo=showcode, cache = TRUE}
...
getBPM = function(x,t1 = 1, sensor = 'IR',chunklength=400, 
		...
		maxBPM = 80,
		Fs = 0.125,  # 0.125 = 8Hz sampling frequency, for BivalveBit boards
...
#		detrendedIR = pracma::detrend(temp[,sensor])
	...
	# Test that detrendedIR now contains no NAs and has some variation (not all 0's)
	...
	# identify the dominant frequency
		...
		y = signal::filtfilt(bf, x = detrendedIR)
		...
		if (plot){
			...
			points(temp$DateTimeMS, detrendedIR, col = 1, pch = 20, cex = 1) # non-filtered data
			...
			points(temp$DateTimeMS[pracPeaks[,2]], y = pracPeaks[,1], col = 4, pch = 19) # mark each peak identified by the pracma routine
			...
			mtext(side = 1, line = 3.5, text = paste(sensor, strftime(temp$DateTimeUTC[1],usetz=TRUE,tz='UTC'),'UTC'), cex = 0.8, adj = 1)
		...
		resultsList = list(DateTime = temp$DateTime[1],
				...
				QA = FALSE)
		# If the slowest heart rate is at least above 4bpm, check and see if there's
		# agreement among the 3 estimates. These algorithms really don't deal well
		# with really slow heart rates, so those slowest values should not be
		# automatically recorded (they should be looked at by a human and confirmed)
		if (min(c(resultsList$BPMpeaks,resultsList$BPMfft,resultsList$BPMforecast)) >= 4){
			...
			if (resultsList$BPMagreeQuality.fft.forecast <= BPMqualityThreshold & 
				...
				resultsList$finalBPM = round(mean(
								c(resultsList$BPMfft,resultsList$BPMforecast,resultsList$BPMpeaks)),  #fixed mean calc 2024-09-25
						digits = 1)
			}
		}
...
for (sn in 1:length(mySNs)){
	...
	for (i in 1:length(bigsteps)){
		...
		res1 = getBPM(SensorData, t1 = bigsteps[i], sensor = 'IR', chunklength = 400,
				lowband = mylowband, highband = myhighband, amplitudeThreshold = 60,
...
```{r reimportBPMfiles,echo=showcode, cache=TRUE}
...
mySN = 'SN189'  # Insert your desired board serial number here
...
while (!quitFlag){
	...
	while (keyPressed != 'q'){
	  ...
	  # and skipFlag is true, handle that case and skip over this time point
		...
		keyPressed = readkeygraph(prompt = "Enter a key: n = next (enters NAs), k = keep [average] 3 BPM values, r = refilter, l = less filtering, p = use pracma bpm, f = use fft bpm, b = step back to previous time, 0 = flatline signal, 2,3,4 = manually enter low rate, s = skip to next, q = quit and save")
		...
		if (keyPressed == 'n'){
		  ...
		  if (HighBandDenom > 5){
			...
			# Use the zero option to denote 'censored' values where the rate
			# is too slow for the sensor + algorithm to effectively detect it
			sensor$finalBPM[thisrow] = 0
			sensor$finalfilter[thisrow] = HighBandDenom
			sensor$QA[thisrow] = TRUE
			keyPressed = 'q' # set this to move on to next time point
		} else if (keyPressed == '2') {
			# Manually enter a value of 2 bpm
			sensor$finalBPM[thisrow] = 2
			sensor$finalfilter[thisrow] = HighBandDenom
			sensor$QA[thisrow] = TRUE
			keyPressed = 'q' # set this to move on to next time point
		} else if (keyPressed == '3') {
			# Manually enter a value of 3 bpm
			sensor$finalBPM[thisrow] = 3
			sensor$finalfilter[thisrow] = HighBandDenom
			sensor$QA[thisrow] = TRUE
			keyPressed = 'q' # set this to move on to next time point
		} else if (keyPressed == '4') {
			# Manually enter a value of 3 bpm
			sensor$finalBPM[thisrow] = 4
			sensor$finalfilter[thisrow] = HighBandDenom
			sensor$QA[thisrow] = TRUE
			keyPressed = 'q' # set this to move on to next time point
		} else if (keyPressed == 's') {
			# Skip to next time point, and also save a copy of the data
#			Heartbpm[which(Heartbpm$Sensor == mysensor),] = sensor
			Heartbpm = sensor
			# Save the data frame to a csv file
			Heartbpm$DateTime = strftime(Heartbpm$DateTime,
					'%Y-%m-%d %H:%M:%S', tz = 'UTC')
			# Save the data frame to a csv file
...
print(paste('Last row checked: ', checkTheseRows[counter], ' time: ', sensor$DateTime[thisrow]))
...
# Output plots Trial 1

```{r openQAdHeartRate,echo=showcode, cache=TRUE}
#Trial 1 consisted of these mussels: 
# Controls, Trial 1:  SN173, SN196, SN224
# Low salinity Trial 1: SN189, SN218, SN179  (SN103 no data)
...
# Reopen the QA'd heart data for which ever SNs have been processed
myBPMs = c('SN173','SN196','SN224','SN189','SN218','SN179','SN214','SN242',
		'SN243','SN198')
...
```{r testPlotTrial1,echo=showcode, cache=TRUE}
# Controls, Trial 1:  SN173, SN196, SN224
# Low salinity Trial 1: SN189, SN218, SN179  (SN103 no data)
...
```{r comboTrial1Plot, echo=showcode, cache=TRUE, dev = 'png',fig.width=12,fig.height=8,dpi=300,fig.cap='Trial 1'}


#Panel the graphs
#layout(matrix(c(1,2,3,4,5,6,7), nrow = 7, ncol = 1),
#		heights = lcm(c(2.5,2.5,2.5,2.5,2.5,3.25,3.5)),respect=TRUE)

layout(matrix(c(1,2,3,4), nrow = 4, ncol = 1),
		heights = lcm(c(2.5,2.5,2.5,3)),respect=TRUE)
par(mar = c(0,6.5,0,1))
mylwd = 2
mycex = 1.25
mygray = 'grey80'
...
#vgrids = as.POSIXct(c('2020-12-29 00:00',
#				'2020-12-30 00:00', '2020-12-31 00:00'), tz = 'etc/GMT+8')
mytimestart = as.POSIXct('2023-08-28 09:00', tz = 'PST8PDT')
mytimeend = as.POSIXct('2023-08-29 15:10', tz = 'PST8PDT')


#plot.new()
par(mar = c(0,12,0.5,4)) # Margins for all plots except the bottom plot
####################################################################
# Plot 1 - Salinity data
tr1 = waterdata[which(waterdata$TrialNumber == '1'),]
tr1 = droplevels(tr1)
...
legend('bottomleft', legend = c('Control','Low salinity'), col = c(4,3),
		lty = c(1,1))
mtext(side = 3, text = 'Trial 1')
###############################################
####################################
...
plot(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, type = 'l', 
		...
		xaxt = 'n',
		ylab = '', axes = FALSE)
axis(side=2, ylim = myBPMylims, col = 2, las = 1, col.axis=2)
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
		col = mygray)
abline(h = 0)
lines(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, col = 2)

oldpar <- par()
par(new=TRUE)
#lines(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = 4)
plot(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = 4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Control, SN173', line = -1.5, cex = 0.8)
#abline(v = c(declineStart, declineFinish,riseStart,riseFinish))
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
abline(h = 0)
lines(SN189bpmQA$DateTime, SN189bpmQA$finalBPM, col = 2)

par(new=TRUE)
#lines(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col =4)
plot(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col =4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)
...
mtext(side = 3, text = 'Low Salinity, SN189', line = -1.5, cex = 0.8)
#abline(v = c(declineStart, declineFinish,riseStart,riseFinish))
###########################################

par(mar = c(1,12,0.5,4)) # Margin for the bottom plot, leave space for x-axis labels
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
abline(h = 0)
lines(SN218bpmQA$DateTime, SN218bpmQA$finalBPM, col = 2)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN218gape$DateTime, SN218gape$gape.mm, type = 'l', col =4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)
#lines(SN218gape$DateTime, SN218gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Low Salinity, SN218', line = -1.5, cex = 0.8)
...
```{r comboTrial1ControlPlot, echo=showcode, cache=TRUE, dev = 'png',fig.width=12,fig.height=8,dpi=300,fig.cap='Trial 1'}
# Controls, Trial 1:  SN173, SN196, SN224
...
legend('bottomleft', legend = c('Salinity'), col = c('royalblue'),
		lty = c(1))
mtext(side = 3, text = 'Trial 1')
abline(v = c(declineStart, declineFinish,riseStart,riseFinish)) 
###############################################
...
###################################
# SN196 Control
plot(SN196bpmQA$DateTime, SN196bpmQA$finalBPM, type = 'l', col = 2,
		ylim = myBPMylims , las = 1,
		ylab = '', axes = FALSE,
		xaxt = 'n')
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
#abline(h = 0)
lines(SN196bpmQA$DateTime, SN196bpmQA$finalBPM, col = 2)

par(new=TRUE)
#lines(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col =4)
plot(SN196gape$DateTime, SN196gape$gape.mm, type = 'l', col =4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)

#lines(SN196gape$DateTime, SN196gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))

mtext(side = 3, text = 'Control, SN196', line = -1.5, cex = 0.8)
abline(v = c(declineStart, declineFinish,riseStart,riseFinish))
###########################################
...
##################################
# SN224 Control
plot(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, type = 'l', col = 4,
		ylim = myBPMylims , las = 1,
		ylab = '', yaxt = 'n')
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
#abline(h = 0)
lines(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, col = 2)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)
#lines(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Control, SN224', line = -1.5, cex = 0.8)
...
```{r comboTrial1LowSalinityPlot, echo=showcode, cache=TRUE, dev = 'png',fig.width=12,fig.height=8,dpi=300,fig.cap='Trial 1'}
# Low salinity Trial 1: SN189, SN218, SN179  (SN103 no data)
...
legend('bottomleft', legend = c('Salinity'), col = c('forestgreen'),
		lty = c(1))
mtext(side = 3, text = 'Trial 1')
abline(v = c(declineStart, declineFinish,riseStart,riseFinish))
###############################################
####################################
# Plot the QA'd heart rate data and gape data for SN189 in Trial 1 Low salinity
# Generally had good well-defined heart signal with high amplitude, although
# there were some sections that seemed arhythmic and had to be scored NA
plot(SN189bpmQA$DateTime, SN189bpmQA$finalBPM, type = 'l', 
		ylim = myBPMylims , las = 1, 
		xaxt = 'n',
		ylab = '', axes = FALSE)
axis(side=2, ylim = myBPMylims, col = 2, las = 1, col.axis=2)
...
###################################
###################################
plot(SN218bpmQA$DateTime, SN218bpmQA$finalBPM, type = 'l', col = 2,
		ylim = myBPMylims , las = 1,
		ylab = '', axes = FALSE,
		xaxt = 'n')
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
...
# interval samples. So I'm almost willing to say that it was doing 1 beat per
# minute for a long time while closed, but I had to assign NAs to those periods
plot(SN179bpmQA$DateTime, SN179bpmQA$finalBPM, type = 'l', col = 4,
		ylim = myBPMylims , las = 1,
		ylab = '', yaxt = 'n')
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = 0.7)
abline(h = 0)
lines(SN179bpmQA$DateTime, SN179bpmQA$finalBPM, col = 2)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN179gape$DateTime, SN179gape$gape.mm, type = 'l', col =4,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = 4, las = 1, col.axis = 4)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = 0.7, col = 4)
#lines(SN179gape$DateTime, SN179gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Low Salinity, SN179', line = -1.5, cex = 0.8)
...
## WSN 2023 poster figure

```{r bigGapeHeartSalinityPlot,echo=showcode, dev = 'png', fig.width=14,fig.height = 20, dpi = 300}
...
#layout(matrix(c(1,2,3,4,5,6,7), nrow = 7, ncol = 1),
#		heights = lcm(c(4.5,4.5,6,4.5,6,4.5,4.5)),respect=TRUE)
layout(matrix(c(1,2,3,4,5,6,7), nrow = 7, ncol = 1),
		heights = lcm(c(6.5,6.5,7,6.5,7.5,6.5,6.5)),respect=TRUE)
par(mar = c(0,6.5,0,1))
mylwd = 2
mycex = 1.25
mycexaxis = 1.5
mygray = 'grey80'
#mygrayblue = rgb(0.82,0.82,0.9)
mygrayblue = 'steelblue1'  # for control plots
#mygraygreen = rgb(0.8,0.85,0.8)  # For low salinity plots
mygraygreen = 'darkseagreen2'  # For low salinity plots
gapecolor = 'gray30'
...
#plot.new()
par(mar = c(0.5,6,0.5,4)) # Margins for all plots except the bottom plot
...
plot(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, type = 'l', 
		...
		xaxt = 'n',
		ylab = '', axes = FALSE)
axis(side=2, ylim = myBPMylims, col = 2, las = 1, col.axis=2, cex.axis = mycexlab2)
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = mycexlab2)
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
		col = mygrayblue)
#abline(h = 0)
SN173bpmQA$finalBPM[which(SN173bpmQA$finalBPM == 0)] <- NA
lines(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, col = 2)

oldpar <- par()
par(new=TRUE)
#lines(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = 4)
plot(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = mycexlab2, col = gapecolor)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Control 38PSU, SN173', line = -1.5, cex = 1, adj = myadj)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = mycexlab2)
#abline(h = 0)
SN196bpmQA$finalBPM[which(SN196bpmQA$finalBPM == 0)] <- NA
lines(SN196bpmQA$DateTime, SN196bpmQA$finalBPM, col = 2)

par(new=TRUE)
#lines(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col =4)
plot(SN196gape$DateTime, SN196gape$gape.mm, type = 'l', col =gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex =  mycexlab2, col = gapecolor)
...
mtext(side = 3, text = 'Control 38PSU, SN196', line = -1.5, cex = 1, adj = myadj)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)
###########################################


par(mar = c(4,6,0.5,4)) # Larger bottom margin
##################################
# SN224 Control
plot(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, type = 'l', col = 4,
		ylim = myBPMylims , las = 1,
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = mycexlab2)
#abline(h = 0)
SN224bpmQA$finalBPM[which(SN224bpmQA$finalBPM == 0)] <- NA
lines(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, col = 2)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '', xlab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = mycexlab2, col = gapecolor)
#lines(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Control 38PSU, SN224', line = -1.5, cex = 1, adj = myadj)

abline(v = c(declineStart, declineFinish,riseStart), lty = 3)


############################################################################
# Now plot the salinity data in the middle panel
par(mar = c(0,6,1.5,4)) # Margins for all plots except the bottom plot
############################################################################
tr1 = waterdata[which(waterdata$TrialNumber == '1'),]
tr1 = droplevels(tr1)
...
for (i in 1:length(levels(tr1$SerialNumber))){
	...
	lines(dat$DateTime, dat$Salinity_psu, col = mycol, lwd = 2)
}
legend('bottomleft', legend = c('Control','Low salinity'), col = c(4,3),
		lty = c(1,1), bty = 'n', lwd = 2)
#mtext(side = 3, text = 'Trial 1')
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)

###########################################

par(mar = c(0.5,6,5,4)) # Margins for all plots except the bottom plot

# Plot the low-salinity mussels
plot(SN189bpmQA$DateTime, SN189bpmQA$finalBPM, type = 'l', 
		ylim = myBPMylims , las = 1, 
		xaxt = 'n',
		ylab = '', axes = FALSE)
...
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		...
		col = mygraygreen)
#abline(h = 0)
SN189bpmQA$finalBPM[which(SN189bpmQA$finalBPM == 0)] <- NA
lines(SN189bpmQA$DateTime, SN189bpmQA$finalBPM, col = 2)

oldpar <- par()
par(new=TRUE)
#lines(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col = 4)
plot(SN189gape$DateTime, SN189gape$gape.mm, type = 'l', col = gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = mycexlab2, col = gapecolor)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Low Salinity, SN189', line = -1.5, cex = 1, adj = myadj)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)
...
#abline(h = 0)
# Censor 0 bpm values
SN218bpmQA$finalBPM[which(SN218bpmQA$finalBPM == 0)] <- NA
lines(SN218bpmQA$DateTime, SN218bpmQA$finalBPM, col = 2)
...
plot(SN218gape$DateTime, SN218gape$gape.mm, type = 'l', col =gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = mycexlab2, col = gapecolor)
...
mtext(side = 3, text = 'Low Salinity, SN218', line = -1.5, cex = 1, adj = myadj)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)
###########################################

par(mar = c(1,6,0.5,4)) # Margin for the bottom plot, leave space for x-axis labels
...
mtext(text='Heart rate, bpm', side = 2, line = 3, col = 2, cex = mycexlab2)
#abline(h = 0)
SN179bpmQA$finalBPM[which(SN179bpmQA$finalBPM == 0)] <- NA
lines(SN179bpmQA$DateTime, SN179bpmQA$finalBPM, col = 2)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN179gape$DateTime, SN179gape$gape.mm, type = 'l', col =gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '')
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = 2.5, text = 'Gape, mm', cex = mycexlab2, col = gapecolor)
#lines(SN179gape$DateTime, SN179gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Low Salinity, SN179', line = -1.5, cex = 1, adj = myadj)
...
```{r bigGapeHeartSalinityPlotPP,echo=showcode, dev = 'png', fig.width=14,fig.height = 20, dpi = 300}
# Show all on one plot
...
par(mar = c(0,6.5,0,1))
mylwd = 2
mycex = 1.75   # 1.25 for poster version
mycexaxis = 2.2  # (time values on x-axis) 1.5 for poster version
mygray = 'grey80'
#mygrayblue = rgb(0.82,0.82,0.9)
mygrayblue = 'steelblue1'  # for control plots
#mygraygreen = rgb(0.8,0.85,0.8)  # For low salinity plots
...
mycexlab = 2.2   # 1.9 for poster version
mycexlab2 = 2.2  # font size for y-axis numbers, 1.4 for poster version
mycexYlabel = 1.9 # font size for y-axis text titles
ylabline = 7 
leftylabline = 4 # alignment for left y-axis text title
rightylabline = 4 # alignment for 2nd y-axis text title
myBPMylims = c(0,34)
myadj = 0.61   # 0.61 for poster version
...
#plot.new()
par(mar = c(0.5,6,0.5,5)) # Margins for all plots except the bottom plot
...
plot(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, type = 'l', 
		...
		ylab = '', axes = FALSE)
axis(side=2, ylim = myBPMylims, col = 2, las = 1, col.axis=2, cex.axis = mycexlab2)
mtext(text='Heart rate, bpm', side = 2, line = leftylabline, col = 2, cex = mycexYlabel)
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
		col = mygrayblue)
#abline(h = 0)
SN173bpmQA$finalBPM[which(SN173bpmQA$finalBPM == 0)] <- NA
lines(SN173bpmQA$DateTime, SN173bpmQA$finalBPM, col = 2, lwd = mylwd)

oldpar <- par()
par(new=TRUE)  # Plot the gape data on the same plot, new y-axis
#lines(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = 4)
plot(SN173gape$DateTime, SN173gape$gape.mm, type = 'l', col = gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '', lwd = mylwd)
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = rightylabline, text = 'Gape, mm', cex = mycexYlabel, col = gapecolor)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
#		lty= c(1,1))
mtext(side = 3, text = 'Control 38PSU, SN173', line = -1.5, cex = 1, adj = myadj)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)
...
#abline(h = 0)
SN196bpmQA$finalBPM[which(SN196bpmQA$finalBPM == 0)] <- NA
lines(SN196bpmQA$DateTime, SN196bpmQA$finalBPM, col = 2, lwd = mylwd)
...
par(mar = c(4,6,0.5,5)) # Larger bottom margin
##################################
# SN224 Control
plot(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, type = 'l', col = 4,
		ylim = myBPMylims , las = 1,
...
#abline(h = 0)
SN224bpmQA$finalBPM[which(SN224bpmQA$finalBPM == 0)] <- NA
lines(SN224bpmQA$DateTime, SN224bpmQA$finalBPM, col = 2, lwd = mylwd)

par(new=TRUE)
# Plot the gape data using a 2nd y-axis
plot(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =gapecolor,
		axes = FALSE, ylim = c(0,8), ylab = '', xlab = '', lwd = mylwd)
axis(side = 4, ylim =c(0,10), col = gapecolor, las = 1, col.axis = gapecolor, cex.axis = mycexlab2)
mtext(side = 4, line = rightylabline, text = 'Gape, mm', cex = mycexYlabel, col = gapecolor)
#lines(SN224gape$DateTime, SN224gape$gape.mm, type = 'l', col =4)
#legend('topleft',legend=c('Heart rate','Gape, mm'), col = c(2,4),
...
############################################################################
# Now plot the salinity data in the middle panel
par(mar = c(0,6,1.5,5)) # Margins for all plots except the bottom plot
############################################################################
tr1 = waterdata[which(waterdata$TrialNumber == '1'),]
tr1 = droplevels(tr1)
...
for (i in 1:length(levels(tr1$SerialNumber))){
	...
	lines(dat$DateTime, dat$Salinity_psu, col = mycol, lwd = mylwd)
}
legend('bottomleft', legend = c('Control','Low salinity'), col = c(4,3),
		lty = c(1,1), bty = 'n', lwd = 4, cex = mycexlab)
mtext(side = 2, text = 'Salinity, psu', line = leftylabline, col = 1, cex = mycexYlabel)
abline(v = c(declineStart, declineFinish,riseStart), lty = 3)

##############################################
# Now plot the low-salinity mussels
##############################################

par(mar = c(0.5,6,5,5)) # Margins for all plots except the bottom plot
...
#abline(h = 0)
SN189bpmQA$finalBPM[which(SN189bpmQA$finalBPM == 0)] <- NA
lines(SN189bpmQA$DateTime, SN189bpmQA$finalBPM, col = 2, lwd = mylwd)
...
# Censor 0 bpm values
SN218bpmQA$finalBPM[which(SN218bpmQA$finalBPM == 0)] <- NA
lines(SN218bpmQA$DateTime, SN218bpmQA$finalBPM, col = 2, lwd = mylwd)
...
par(mar = c(1,6,0.5,5)) # Margin for the bottom plot, leave space for x-axis labels
...
#abline(h = 0)
SN179bpmQA$finalBPM[which(SN179bpmQA$finalBPM == 0)] <- NA
lines(SN179bpmQA$DateTime, SN179bpmQA$finalBPM, col = 2, lwd = mylwd)
...
## Example field salinity data
```{r loadCTDOmatFiles, echo=showcode}
# Some example salinity data from the 2019-2020 LPL deployment (with oysters)

# Directory with CTDO sensor data from the oyster mooring in LPL
CTDOdir = '../../data/Weather_water_data/LPL_mooring_data_2019-2020/CTDOs/LPL_oyster_mooring_CTDOs/'
CTDObottomFile = 'SBE37SMP-ODO-RS232_03713802_2020_05_11_v6.mat'
CTDOsurfaceFile = 'SBE37SMP-ODO-RS232_03714707_2020_05_14_v6.mat'

# Make sure package R.matlab has been loaded
# Import bottom-mounted CTD data
ctdBottom = R.matlab::readMat(paste0(CTDOdir,CTDObottomFile))
ctdBottom$DateTimeUTC = as.POSIXct((ctdBottom$dateUTC - 719529)*86400, 
		origin = "1970-01-01", tz = "UTC")
ctdBottom = ctdBottom[c('DateTimeUTC','pres','con','sal','temp','O2mgl',
				'O2perc','depth','density')]
ctdBottom$DateTimePST = ctdBottom$DateTimeUTC
attr(ctdBottom$DateTimePST,'tzone') = 'etc/GMT+8'
ctdBottom = as.data.frame(ctdBottom)

# Import surface CTD data
ctdSurface = R.matlab::readMat(paste0(CTDOdir, CTDOsurfaceFile))
ctdSurface$DateTimeUTC = as.POSIXct((ctdSurface$dateUTC - 719529)*86400, 
		origin = "1970-01-01", tz = "UTC")
ctdSurface = ctdSurface[c('DateTimeUTC','pres','con','sal','temp','O2mgl',
				'O2perc','depth','density')]
ctdSurface$DateTimePST = ctdSurface$DateTimeUTC
attr(ctdSurface$DateTimePST,'tzone') = 'etc/GMT+8'
ctdSurface = as.data.frame(ctdSurface)

# Look at the period 2020-03-12 12:30:02 PST thru 2020-03-12 17:15:02 for
# example of a very fast drop from 31PSU to 3PSU over ~5 hours

# 2020-03-17 10:55 (29PSU) to 2020-03-17 19:45 (1.1PSU) ~9 hours

# 2020-03-23 13:55 (31PSU) to 2020-03-23 18:50 (2.6PSU) ~5 hours

# Also look at the ctdBottom data on 2019-12-26 between ~10AM and 14:00 for
# a super-fast drop from 30 to ~1PSU over the course of about 4 hours.

```


```{r ctdPlot1,echo=showcode}
myd1 = as.POSIXct('2020-03-17 09:00', tz='etc/GMT+8')
myd2 = as.POSIXct('2020-03-19 21:00', tz='etc/GMT+8')

temp = ctdSurface[ctdSurface$DateTimePST >= myd1 & ctdSurface$DateTimePST <= myd2,]

plot(x = temp$DateTimePST, y = temp$sal, type = 'l')


```


```{r ctdPlot2,echo=showcode}
myd1 = as.POSIXct('2020-03-23 13:00', tz='etc/GMT+8')
myd2 = as.POSIXct('2020-03-24 21:00', tz='etc/GMT+8')

temp = ctdSurface[ctdSurface$DateTimePST >= myd1 & ctdSurface$DateTimePST <= myd2,]

plot(x = temp$DateTimePST, y = temp$sal, type = 'l', las = 1,
		xlab = 'Time', ylab = 'Salinity, PSU')
```

```{r LPLSalinityDec2019,echo=showcode, dev = 'png', fig.width = 7, fig.height = 4, dpi = 300}
# LPL December 2019 following a rainstorm
myd1 = as.POSIXct('2019-12-26 08:00', tz='etc/GMT+8')
myd2 = as.POSIXct('2019-12-27 14:00', tz='etc/GMT+8')

temp = ctdSurface[ctdSurface$DateTimePST >= myd1 & ctdSurface$DateTimePST <= myd2,]

plot(x = temp$DateTimePST, y = temp$sal, type = 'l', las = 1,
		xlab = 'Time', ylab = 'Salinity, PSU', xaxs = 'i', lwd = 2)
rect(xleft = par()$usr[1],ybottom=par()$usr[3],
		xright=par()$usr[2],ytop=par()$usr[4],
		col = mygray)
#abline(v = myseq, lty = 3, col = 'white')
mtext(side = 3, text = 'Los Penasquitos Lagoon surface water, Dec 26-27 2019')
lines(x = temp$DateTimePST, y = temp$sal, lwd = 2)
myseq = seq(from = myd1, by = 3600, length.out = 32)

```

```{r averageRates,echo=showcode}

hourbefore = as.POSIXct('2023-08-28 13:20', tz='PST8PDT')
declineStart = as.POSIXct('2023-08-28 14:20',tz='PST8PDT')
declineFinish = as.POSIXct('2023-08-28 22:30',tz='PST8PDT')
riseStart = as.POSIXct('2023-08-29 11:40',tz='PST8PDT')
riseFinish= as.POSIXct('2023-08-29 15:10',tz='PST8PDT')
midstart = as.POSIXct('2023-08-29 4:40', tz = 'PST8PDT')
midend = as.POSIXct('2023-08-29 5:40', tz = 'PST8PDT')
lasthour = as.POSIXct('2023-08-29 14:10', tz='PST8PDT')

# Compare average gape in the hour before the start of the decline

controls = c('SN173', 'SN196','SN224')
lowsal = c('SN189','SN218','SN179')


summaryfunc = function(ID = 'SN173', bpmData, gapeData, Phase = NULL, Treatment = NULL, startTime,endTime){
	meanbpm = mean(bpmData[bpmData$DateTime > startTime & 
							bpmData$DateTime < endTime, 'finalBPM'], na.rm = TRUE)
	meangape = mean(gapeData[gapeData$DateTime > startTime & 
							gapeData$DateTime < endTime, 'gape.mm'], na.rm = TRUE)
	tempdf = data.frame(ID = ID, Treatment = Treatment, Phase = Phase, AvgBPM = meanbpm, 
			AvgGape = meangape)
	tempdf
}

# Control mussels
res = summaryfunc(ID = 'SN173', SN173bpmQA, SN173gape, Phase = 'start', 'Control', hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN173', SN173bpmQA, SN173gape, Phase = 'middle','Control',  midstart, midend)
res3 = summaryfunc(ID = 'SN173', SN173bpmQA, SN173gape, Phase = 'end','Control',  lasthour, riseFinish)
responsesummary = rbind(res,res2,res3)


res = summaryfunc(ID = 'SN196', SN196bpmQA, SN196gape, Phase = 'start','Control',  hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN196', SN196bpmQA, SN196gape, Phase = 'middle', 'Control', midstart, midend)
res3 = summaryfunc(ID = 'SN196', SN196bpmQA, SN196gape, Phase = 'end','Control',  lasthour, riseFinish)
responsesummary = rbind(responsesummary, res,res2,res3)

res = summaryfunc(ID = 'SN224', SN224bpmQA, SN224gape, Phase = 'start','Control',  hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN224', SN224bpmQA, SN224gape, Phase = 'middle', 'Control', midstart, midend)
res3 = summaryfunc(ID = 'SN224', SN224bpmQA, SN224gape, Phase = 'end','Control',  lasthour, riseFinish)
responsesummary = rbind(responsesummary, res,res2,res3)

# Low salinity mussels
res = summaryfunc(ID = 'SN189', SN189bpmQA, SN189gape, Phase = 'start', 'Low Salinity', hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN189', SN189bpmQA, SN189gape, Phase = 'middle','Low Salinity',  midstart, midend)
res3 = summaryfunc(ID = 'SN189', SN189bpmQA, SN189gape, Phase = 'end','Low Salinity',  lasthour, riseFinish)
responsesummary = rbind(responsesummary, res,res2,res3)

res = summaryfunc(ID = 'SN218', SN218bpmQA, SN218gape, Phase = 'start','Low Salinity',  hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN218', SN218bpmQA, SN218gape, Phase = 'middle', 'Low Salinity', midstart, midend)
res3 = summaryfunc(ID = 'SN218', SN218bpmQA, SN218gape, Phase = 'end','Low Salinity',  lasthour, riseFinish)
responsesummary = rbind(responsesummary, res,res2,res3)

res = summaryfunc(ID = 'SN179', SN179bpmQA, SN179gape, Phase = 'start','Low Salinity',  hourbefore, declineStart)
res2 = summaryfunc(ID = 'SN179', SN179bpmQA, SN179gape, Phase = 'middle', 'Low Salinity', midstart, midend)
res3 = summaryfunc(ID = 'SN179', SN179bpmQA, SN179gape, Phase = 'end','Low Salinity',  lasthour, riseFinish)
responsesummary = rbind(responsesummary, res,res2,res3)

responsesummary$Treatment = factor(responsesummary$Treatment)
responsesummary$Phase = factor(responsesummary$Phase, levels = c('start','middle','end'))
responsesummary$ID = factor(responsesummary$ID)

```


```{r heartresponsePlot, echo=showcode}
library(ggplot2)
library(reshape2)

ggplot(responsesummary, aes(x = Phase, y = AvgBPM)) +
		geom_point(aes(color = Treatment), shape = 20, size = 4, position = position_dodge(width = 0.4)) +
		theme_bw()
```


```{r gapeResponsePlot, echo=showcode}
ggplot(responsesummary, aes(x = Phase, y = AvgGape)) +
		geom_point(aes(color = Treatment), shape = 20, size = 4, position = position_dodge(width = 0.4)) +
		theme_bw()
```
